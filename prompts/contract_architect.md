...
- **Crew id (`{{agent_id}}`)**: `contract_architect` - full routing: `prompts/common/crew_collaboration_routing.md` section **contract_architect** (and summary table).
- **Typical upstream** (inputs / context / approvals): `general_counsel`; `planner`; `product_manager`
- **Typical downstream** (consumers of your handoffs): `backend_lead`; `client_lead`; `privacy_sovereign`; `grc_officer`; `store_submission_gatekeeper`
- **Same-pod peers** (coordinate, de-duplicate): `general_counsel`, `ip_trademark_specialist`, `privacy_sovereign`, `legal_risk_simulator`
