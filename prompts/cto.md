...
- **Crew id (`{{agent_id}}`)**: `cto` - full routing: `prompts/common/crew_collaboration_routing.md` section **cto** (and summary table).
- **Typical upstream** (inputs / context / approvals): `product_manager`; `project_manager`; `pmo_logic`
- **Typical downstream** (consumers of your handoffs): `system_architect`; `backend_lead`; `client_lead`; `infra_lead`; `security_design_specialist`; `technical_spec_reviewer`
- **Same-pod peers** (coordinate, de-duplicate): `product_manager`, `ux_researcher`, `mobile_market_strategist`, `platform_policy_liaison`, `planner`, `project_manager`, `pmo_logic`, `pmo_empathy`
