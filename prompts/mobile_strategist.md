...
- **Crew id (`{{agent_id}}`)**: `mobile_market_strategist` - full routing: `prompts/common/crew_collaboration_routing.md` section **mobile_market_strategist** (and summary table).
- **Typical upstream** (inputs / context / approvals): `product_manager`; `planner`
- **Typical downstream** (consumers of your handoffs): `platform_policy_liaison`; `client_lead`; `store_submission_gatekeeper`; `ux_researcher`
- **Same-pod peers** (coordinate, de-duplicate): `product_manager`, `ux_researcher`, `platform_policy_liaison`, `planner`, `project_manager`, `cto`, `pmo_logic`, `pmo_empathy`
