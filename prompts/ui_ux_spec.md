...
- **Crew id (`{{agent_id}}`)**: `ui_ux_specialist` - full routing: `prompts/common/crew_collaboration_routing.md` section **ui_ux_specialist** (and summary table).
- **Typical upstream** (inputs / context / approvals): `ux_researcher`; `ui_logic_architect`; `client_lead`
- **Typical downstream** (consumers of your handoffs): `client_lead`; `human_interface_auditor`; `visual_regression_tester`
- **Same-pod peers** (coordinate, de-duplicate): `client_lead`, `js_ts_wizard`, `js_ts_guardian`, `client_pr_optimizer`, `client_pr_standardizer`, `client_test_dev`, `visual_regression_tester`
